...
public:
  ...
  void Clear(int value) const { memset(bufBegin, value, BufferByteSize()); }
      ...
      elemPaddedSize{ComputePaddedSize(sizeof(ElemType), alignment)}
{
  ...
  DX::ThrowIfFailed(Resource()->Map(0, &range, &bufBegin));
}
...
{
  auto* dst = static_cast<UINT8*>(bufBegin) + byteOffset;
...
{
  auto* p = static_cast<UINT8*>(bufBegin);
  p += index * elemPaddedSize;
